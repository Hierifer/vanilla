...
# 追加：禁止对本地地址使用系统代理
import os as _os
_no_proxy_val = '0.0.0.0,127.0.0.1'
...
class LLMSessionManager:
    ...
    async def start_session(self, websocket: WebSocket, new=False):
        ...
        try:
            ...
            print(f"http://0.0.0.0:{self.memory_server_port}/new_dialog/{self.lanlan_name}")
    ...
    async def _background_prepare_pending_session(self):
        ...
        try:
            ...
            async with httpx.AsyncClient(trust_env=False) as client:
                resp = await client.get(f"http://0.0.0.0:{self.memory_server_port}/new_dialog/{self.lanlan_name}", timeout=5)
