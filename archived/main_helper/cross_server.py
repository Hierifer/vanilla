...
def sync_connector_process(message_queue, shutdown_event, lanlan_name, sync_server_url=f"ws://0.0.0.0:{MONITOR_SERVER_PORT}", config=None):
    """独立进程运行的同步连接器"""
                ...
                if config['bullet']:
                    if bullet_ws is None or bullet_ws.closed:
                        ...
                        bullet_ws = await bullet_session.ws_connect(
                            f"wss://0.0.0.0:{COMMENTER_SERVER_PORT}/sync/{lanlan_name}",
                        ...
                        try:
                            ...
                            if message["data"] == "renew session":
                                ...
                                response = requests.post(
                                    f"http://0.0.0.0:{MEMORY_SERVER_PORT}/renew/{lanlan_name}",
                            ...
                            if message["data"] == 'turn end': # lanlan的消息结束了
                                ...
                                print("💗开始处理聊天历史")
                                response = requests.post(
                                    f"http://0.0.0.0:{MEMORY_SERVER_PORT}/process/{lanlan_name}",
                                    json={'input_history': json.dumps(chat_history, indent=2, ensure_ascii=False)},
                                )
                                if response.json()['status'] == 'error':
                                    print("💥 Conversation processing error", response.json()['message'])
